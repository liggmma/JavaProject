                        ...
                        <c:forEach var="conv" items="${conversations}">
                            <a href="chat?id=${conv.conversationId}" style="text-decoration: none; color: inherit;">
                                <div class="conversation-item ${conv.conversationId == currentConversationId ? 'active' : ''}">
                                    <div class="avatar">
                                        <c:choose>
                                            <c:when test="${not empty conv.shopId.shopLogoUrl}">
                                                <img src="${conv.shopId.shopLogoUrl}" alt="Avatar">
                                            </c:when>
                                            <c:otherwise>
                                                <img src="${pageContext.request.contextPath}/images/shop_default.png" alt="Avatar">
                                            </c:otherwise>
                                        </c:choose>
                                    </div>
                                    <div class="details">
                                        <h4 class="name">${conv.shopId.shopName}</h4>
                                        <c:choose>
                                    ...
                                    <div class="meta">
                                        ...
                                        <c:if test="${conv.shopUnReadCount > 0}">
                                            <div class="unread-badge">${conv.shopUnReadCount}</div>
                                        </c:if>
                                    </div>
                                </div>
                            </a>
                    ...
                    <c:choose>
                        <c:when test="${not empty currentConversation}">
                            <header class="chat-header">
                                <div class="avatar">
                                    <c:choose>
                                        <c:when test="${not empty currentConversation.shopId.shopLogoUrl}">
                                            <img src="${currentConversation.shopId.shopLogoUrl}" alt="Avatar">
                                        </c:when>
                                        <c:otherwise>
                                ...
                                <div class="name">${currentConversation.shopId.shopName}</div>
                            </header>

                            <div class="message-area" id="messageArea">
                                <c:forEach var="msg" items="${currentConversation.messagesList}">
                                    <div class="message ${msg.senderType == 'user' ? 'outgoing' : 'incoming'}">
