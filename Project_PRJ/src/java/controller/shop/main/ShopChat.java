 ...
 * @author ADMIN
 */
@WebServlet(name = "ShopChat", urlPatterns = {"/shop/chat"})
public class ShopChat extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Conversations> conversations = conversationService.getConversationForShop(shopId);
        ...
        if (conversationIdParam != null) {
            try {
                ...
                for (Messages m : conversation.getMessagesList()) {
                    if (m.getSenderType().equalsIgnoreCase("user")) {
        ...
        request.getRequestDispatcher("/shop/shop_chat.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        HttpSession session = request.getSession();
        Users shopUser = (Users) session.getAttribute("user");

        String content = request.getParameter("messageContent");
        String conversationIdStr = request.getParameter("conversationId");

        // 2. Kiểm tra dữ liệu đầu vào
        if (shopUser == null || content == null || content.trim().isEmpty() || conversationIdStr == null) {
        ...
        try {
            ...
            newMessage.setSenderType("shop"); // Người gửi là shop
            newMessage.setMessageType("text");
            newMessage.setIsRead(Boolean.FALSE);
            newMessage.setSenderId(shopUser.getUserId()); // ID của người gửi (chủ shop)
